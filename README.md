...
## Ограничения
Этот метод ограничен приложениями на системах и устройствах, где вы можете осуществить перенаправление на фейковый сервер активации и установить сертификат фейкового корневого центра сертификации.
...
Дождитесь пока сервер перезапуститься. Откройте панель управления сервера, вы должны увидеть золотую медаль Emby Premiere.
...
Если вы не видите такой же картины в своем браузере, и на Шаге 1 и 2 нет проблем, проверьте не используете ли вы какие-либо прокси-плагины или программы прокси, отключите их, перезапустите браузер и повторите попытку. Если все еще не работает, перейдите в `Параметры Windows` - `Сеть и Интернет` - `Прокси-сервер` и убедитесь что переключатель `Использовать прокси-сервер` выключен.

![i-step3-2](images/i-step3-2.png)

> Это не значит, что нельзя использовать эту программу совместно с прокси-сервером, нужно лишь добавить адрес `mb3admin.com` в исключения вашего прокси-сервера, чтобы он открывался напрямую, в обход вашего прокси. 

#### ШАГ4：Все еще не работает？

Тогда вам, увы, придется решать это самостоятельно.
