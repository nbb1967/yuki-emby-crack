...
> Данный репозиторий является ответвлением проекта ув. MitsuhaYuki [yuki-emby-crack](https://github.com/MitsuhaYuki/yuki-emby-crack), служит целям локализации, уточнения справочной информации, актуализации сертификатов "для ленивых" и **НЕ СОДЕРЖИТ ИЗМЕНЕНИЙ В КОДЕ ПРОГРАММЫ!**
...
> Это не значит, что нельзя использовать эту программу совместно с прокси-сервером, нужно лишь добавить адрес `mb3admin.com` в исключения прокси-сервера, чтобы он открывался напрямую, в обход прокси-сервера. 
