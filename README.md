...
> **Предупреждение**: использование чужого сертификата в качестве доверенного корневого удостоверяющего центра небезопасно. Рекомендуется выпустить свой собственный сертификат!

> Данный репозиторий является форком проекта MitsuhaYuki [yuki-emby-crack](https://github.com/MitsuhaYuki/yuki-emby-crack), служит целям локализации, уточнения справочной информации, актуализации сертификатов "для ленивых" и не содержит изменений в коде программы!

## Принцип работы
Подмена сервера активации приложений Emby вашим собственным фейковым сервером активации приложений.

## Ограничения
Этот метод ограничен приложениями на системах и устройствах, где вы можете осуществить перенаправление на фейковый сервер активации и установить сертификат фейкового центра сертификации.

## Результат

> Активированный браузер

![i-demo-1](images/i-demo-1.png)

> Активированный Emby Theater
...
Скачайте `dist.zip` со страницы [`release`](https://github.com/nbb1967/yuki-emby-crack/releases) и разархивируйте его в вашу папку `Документы`: `C:\Users\<UserName>\Documents\dist`

![i-dist.png](images/i-unzip.png)

Запустите `main.exe`，в Windows может появиться запрос Брандмауэра защитника Windows，разрешите оба.

> вы можете изменить расположение программы，но требуется поместить ее в вашу личную папку，иначе она может не запускаться из-за недостаточных прав доступа

### 2. Настройте автоматический запуск

Щелкните правой кнопкой по `main.exe`，и выберите `Создать ярлык`，переместите `main.exe - ярлык` в папку `C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp`
...
> Программа, которой вы редактируете файл `hosts` (например, `Блокнот`) должна быть запущена от имени администратора
...
Щелкните правой кнопкой мыши по значку Emby Server в панели уведомлений и выберите `Перезапуск Emby Server`
Дождитесь пока сервер перезапуститься. Откройте панель управления сервером, вы уже должны видеть золотую медаль Emby Premiere.
